...
from app.models import (
    ModelPrice, PriceStatus, User, SystemSetting, 
    Provider, ProviderStatus, StandardModel, StandardModelRequest
)
...
async def get_pending_prices(
    ...
    statement = select(ModelPrice, Provider, StandardModel).join(
        Provider, ModelPrice.provider_id == Provider.id
    ).join(
        StandardModel, ModelPrice.standard_model_id == StandardModel.id
...
async def get_pending_providers(
    ...
    statement = select(Provider, User).join(
        User, Provider.owner_id == User.id
...
async def get_pending_model_requests(
    ...
    statement = select(StandardModelRequest, User).join(
        User, StandardModelRequest.requester_id == User.id
